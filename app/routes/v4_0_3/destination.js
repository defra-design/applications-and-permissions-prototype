...
module.exports = function (router) {
    ...
    router.post( section + 'type-of-destination-router', function (req, res)
    {
        ...
        {
            if (req.session.data['origin-to-or-from-own-premises-radios'] == "On to the farm or premises"  &&
                req.session.data['origin-type-of-origin-on-radios'] == "Approved finishing units (AFU)")
            {
    ...
    router.post(section + 'date-of-movement-router', function (req, res)
    {
        ////////////////////////////////////////////////////////////////////////////////////
        ...
        if ( req.session.data['destination-date-of-movement-date-input-day'] == undefined
            || req.session.data['destination-date-of-movement-date-input-day'] == "" )
        {
            dayEmpty = true;
        }
        // Validation check if month field is blank
        if ( req.session.data['destination-date-of-movement-date-input-month'] == undefined
            || req.session.data['destination-date-of-movement-date-input-month'] == "" )
        {
            monthEmpty = true;
        }
        // Validation check if year field is blank
        if ( req.session.data['destination-date-of-movement-date-input-year'] == undefined
            || req.session.data['destination-date-of-movement-date-input-year'] == "" )
        {
            yearEmpty = true;
        }
        ...
        if (req.session.data['errorthispage'] != "true")
        {
            ...
            if (  isNaN(req.session.data['destination-date-of-movement-date-input-year']) )
            {
        ...
        if (req.session.data['errorthispage'] != "true")
        {
            if (  req.session.data['destination-date-of-movement-date-input-year'] < 1000  ||  9999 < req.session.data['destination-date-of-movement-date-input-year']  )
            {
                req.session.data['errorthispage'] = "true";
        ...
        {
            // if no error have been found so far then check for non numbers
            if ( isNaN(req.session.data['destination-date-of-movement-date-input-month']) )
            {
                // one or more fields isn't a number and isn't empty
                req.session.data['errorthispage'] = "true";
            ...
            // Check for non numbers being entered
            else if ( req.session.data['destination-date-of-movement-date-input-month'] < 1  ||  12 < req.session.data['destination-date-of-movement-date-input-month'] )
            {
                // one or more fields isn't a number and isn't empty
                req.session.data['errorthispage'] = "true";
                req.session.data['errortypeten'] = "true";
        ...
        if (req.session.data['errorthispage'] != "true")
        {
            var quanityofdaysinmonth =  new Date(req.session.data['destination-date-of-movement-date-input-year'], req.session.data['destination-date-of-movement-date-input-month'], 0).getDate();

            // if no error have been found so far then check for non numbers
            if ( isNaN(req.session.data['destination-date-of-movement-date-input-day'])  )
            {
                // one or more fields isn't a number and isn't empty
                req.session.data['errorthispage'] = "true";
                ...
                // Check if date numbers are 0 or impossibly high. e.g. 14th month
            // Check for non numbers being entered
            else if (  req.session.data['destination-date-of-movement-date-input-day'] < 1  ||  quanityofdaysinmonth < req.session.data['destination-date-of-movement-date-input-day'] )
            {
                // one or more fields isn't a number and isn't empty
                req.session.data['errorthispage'] = "true";
                req.session.data['errortypeeleven'] = "true";
        ...
        if (req.session.data['errorthispage'] != "true")
        {
            inputdate = new Date(
                req.session.data['destination-date-of-movement-date-input-year'],
                req.session.data['destination-date-of-movement-date-input-month'] - 1,
                req.session.data['destination-date-of-movement-date-input-day']
            );

            // Save user input date without zeros and month has taxt, e.g. March
            req.session.data['destination-date-of-movement-date-input-day'] = inputdate.getDate();
            req.session.data['destination-date-of-movement-date-input-month-number'] = inputdate.getMonth() + 1;
            req.session.data['destination-date-of-movement-date-input-month-text'] = inputdate.toLocaleString('default', {month: 'long'});
            req.session.data['destination-date-of-movement-date-input-year'] = inputdate.getFullYear();
        }
